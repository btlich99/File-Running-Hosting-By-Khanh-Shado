...
const handleReactionNew = require("./handle/handleReactionNew")({ api }); 
	...
	logger.loader(`Ping load toàn bộ server Bot: ${Date.now() - global.client.timeStart}ms`);
  logger.loader(`Ping load toàn bộ module(s) command và module(s) events: ${Date.now() - global.client.timeStart}ms`);
	...
	const checkAndExecuteEvent = async () => {
		...
		//GET CURRENT TIME
		var timeVN = moment().tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY ||_HH:mm:ss');
	...
	return async (event) => {
            ...
            if (!data.includes(event.threadID) && !ADMINBOT.includes(event.senderID) && global.config["duyet"].status == true) {
                ...
                //check body
                if (event.body && event.body == `Zeus_Project`) {
                    ADMINBOT.forEach(e => {
                ...
                if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`Nhóm bạn chưa được Admin duyệt. Để gửi yêu cầu duyệt, dùng: Zeus_Project`, event.threadID);
            };
		switch (event.type) {
			case "message":
			case "message_reply":
