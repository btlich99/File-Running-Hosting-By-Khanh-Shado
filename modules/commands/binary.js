...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (type == "enc" || type == "e") return api.sendMessage(`${encode(input)}`, threadID, messageID);
  if (type == "dec" || type == "d") return api.sendMessage(`${decode(input)}`, threadID, messageID);
}