...
var coinscn = Math.floor(Math.random() * 90000) + 1; //random coins khi làm ở khu công nghiệp
var coinsdv = Math.floor(Math.random() * 500000) + 1; //random coins khi làm ở khu dịch vụ
var coinsmd = Math.floor(Math.random() * 1500000) + 1; //random coins khi làm ở mỏ dầu
var coinsq = Math.floor(Math.random() * 3000000) + 1; //random coins khi khai thác quặng
var coinsdd = Math.floor(Math.random() * 4509000) + 1; //random coins khi đào đá
var coinsdd1 = Math.floor(Math.random() * 5005000) + 1; //random coins khi đào đá
var coinstb = Math.floor(Math.random() * 6000800) + 1; //random coins khi kinh doanh thiết bị
var coinsqt = Math.floor(Math.random() * 7061000) + 1; //random quân trang
var coinsso = Math.floor(Math.random() * 8000800) + 1; //random duễn viên nhật :)))
var coincs = Math.floor(Math.random() * 9500300) + 1; //random chat sex
...
var work9 = rddso[Math.floor(Math.random() * rddso.length)];
  ...
  var rdcs = ['người yêu', 'bà cô trung niên', 'bác đại gia', 'cô bé lọ lem', 'gái chưa 18'];
  var work10 = rdcs[Math.floor(Math.random() * rdcs.length)]; //random công việc khi chat sex
    ...
    switch(handleReply.type) {
        case "choosee": {
            ...
            switch(event.body) {
                ...
                case "10": msg = `😱 Bạn làm công việc chat sex với ${work10} và nhận về số tiền là ${coincs}$` ; Currencies.increaseMoney(event.senderID, coincs);
break; //thêm case nếu muốn 
            ...
            if (choose > 10 || choose < 1) return api.sendMessage("⚡️ Lựa chọn không nằm trong danh sách.", event.threadID, event.messageID); //thay số case vào số 7
